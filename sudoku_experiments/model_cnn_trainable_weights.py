# Convolutional Neural Network to classify a sudoku as valid or invalid. The weights are not fixed.
...
#weights_1 = np.ones(shape=(3, 3, 1, 1))
#noise_1 = np.random.normal(0, 1, weights_1.shape)
#signal_1 = weights_1 + noise_1
#
#weights_2 = np.ones(shape=(1, 9, 1, 1))
#noise_2 = np.random.normal(0, 1, weights_2.shape)
#signal_2 = weights_2 + noise_2
#
#weights_3 = np.ones(shape=(9, 1, 1, 1))
#noise_3 = np.random.normal(0, 1, weights_3.shape)
#signal_3 = weights_3 + noise_3
...
model_log=model.fit([X_train]*3, y_train, epochs=10, batch_size=200, validation_split=0.10,
...
for i in range(3):
    weights = model.get_layer('conv2d_'+str((i+1))).get_weights()
    print(weights)
